...
class Plain1Selection():

    @staticmethod
    def getFileName(param_dict):
        return 'Plain1Selection_PG-{}'.format(param_dict['PG'].getFileName(param_dict['PG_param']))
    ...
    def classifyAllLabels(self, X_test, X_file, y_file, X_train, y_train, n_labels, n_neigh):
        ...
        for it_label in range(n_labels):
            ...
            X_red, y_red = self.closestElement(X_train, y_train[:, it_label], X_red, y_red)
...
if __name__ == '__main__':
    ...
    X_red, y_red = Plain1Selection().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)
