...
class Metamethod1Dependent():

    @staticmethod
    def getFileName(param_dict):
        return 'Metamethod1Dependent_PG-{}'.format(param_dict['PG'].getFileName(param_dict['PG_param']))
    ...
    def reduceSet(self, X, y, param_dict):
        ...
        # Voting stage:
        votes = np.zeros(shape=self.y.shape[0], dtype=np.int)
        for it_label in range(self.y.shape[1]):
            # print("Processing label #{}".format(it_label))

            # Obtaining current label annotations:
            y_current = self.y[:, it_label]

            # Obtaining additional features:
            additional_features = list(range(y.shape[1]))
            additional_features.pop(it_label)

            # Including additional features:
            X_aux = np.append(self.X, self.y[:, additional_features], axis = 1)

            # Using a PS method:
            Xred, yred = self.PG.reduceSet(X_aux, y_current, self.NN)

            # 
            selection = self.closestElement(X_aux, y_current, Xred, yred)
        ...
        for u in range(y_T.shape[1]-1):
            ...
            # print("u: {} - error_u: {} - m_u: {}".format(u, error_u, m_u))
...
class Metamethod1():
    ...
    @staticmethod
    def getFileName(param_dict):
        return 'Metamethod1_PG-{}'.format(param_dict['PG'].getFileName(param_dict['PG_param']))
    ...
    def reduceSet(self, X, y, param_dict):
        ...
        for it_label in range(self.y.shape[1]):
            ...
            # Using a PG method:
            Xred, yred = self.PG.reduceSet(self.X, y_current, self.PG_param)
            
            # 
            selection = self.closestElement(self.X, y_current, Xred, yred)
...
if __name__ == '__main__':
    ...
    params_dict = Metamethod1.getParameterDictionary()
    X_red, y_red = Metamethod1().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)
    params_dict = Metamethod1Dependent.getParameterDictionary()
    X_redDep, y_redDep = Metamethod1Dependent().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)


    cls_ori = BRkNNaClassifier(k=1).fit(X_train, y_train)
    cls_red = BRkNNaClassifier(k=1).fit(X_red, y_red)
    cls_redDep = BRkNNaClassifier(k=1).fit(X_redDep, y_redDep)

    y_pred_ori = cls_ori.predict(X_test)
    y_pred_red = cls_red.predict(X_test)
    y_pred_redDep = cls_red.predict(X_test)
    ...
    print("\t - Hamming Loss: {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_redDep), 100*X_redDep.shape[0]/X_train.shape[0]))
    print("Done!")